...
**Decisão:** detectar duplicados **entre fontes** no front (`assets/app.js`, `calcularDuplicados`) pela chave `(área arredondada, preço)`, considerando só grupos que abrangem ≥2 fontes distintas (colisões dentro da mesma fonte são unidades diferentes). O arredondamento da área usa a regra **meio-para-cima explícita** `floor(x+0,5)`, idêntica no dashboard (`arredondarArea`) e no validador (`validate_data.py`) — o `round()` do Python usa banker's rounding e divergiria em áreas `.5`. Os **KPIs e o scatter** contam cada grupo uma vez (via `semDuplicados`); a **tabela mostra todas as linhas**, marcando as duplicadas com selo `↔ também em {fonte}` e estilo discreto. Nada é removido dos dados nem da tabela.
